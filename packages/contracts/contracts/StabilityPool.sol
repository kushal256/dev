 ...
 * as 0, since it is now less than 1e-9'th of its initial value (e.g. a deposit of 1 billion Debt has depleted to < 1 Debt).
 *
 *
...
contract StabilityPool is LiquityBase, Ownable, CheckContract, IStabilityPool {
    ...
    uint256 internal totalDebtDeposits;
    ...
    * after a series of liquidations have occurred, each of which cancel some debt debt with the deposit.
    *
    ...
    */
    ...
    uint public lastDebtLossError_Offset;
    ...
    event CollateralGainWithdrawn(address indexed _depositor, uint _Collateral, uint _DebtLoss);
    ...
    */
    function provideToSP(uint _amount, address _frontEndTag) external override {
        ...
        uint depositorETHGain = getDepositorCollateralGain(msg.sender);
        uint compoundedDebtDeposit = getCompoundedDebtDeposit(msg.sender);
        uint debtLoss = initialDeposit.sub(compoundedDebtDeposit); // Needed only for event log
        ...
        _sendDebttoStabilityPool(msg.sender, _amount);

        uint newDeposit = compoundedDebtDeposit.add(_amount);
        _updateDepositAndSnapshots(msg.sender, newDeposit);
        emit UserDepositChanged(msg.sender, newDeposit);

        emit CollateralGainWithdrawn(msg.sender, depositorETHGain, debtLoss); // Debt Loss required for event log
    ...
    */
    function withdrawFromSP(uint _amount) external override {
        ...
        uint compoundedDebtDeposit = getCompoundedDebtDeposit(msg.sender);
        uint debtToWithdraw = LiquityMath._min(_amount, compoundedDebtDeposit);
        uint debtLoss = initialDeposit.sub(compoundedDebtDeposit); // Needed only for event log
        ...
        // Update front end stake
        uint compoundedFrontEndStake = getCompoundedFrontEndStake(frontEnd);
        uint newFrontEndStake = compoundedFrontEndStake.sub(debtToWithdraw);
        _updateFrontEndStakeAndSnapshots(frontEnd, newFrontEndStake);
        emit FrontEndStakeChanged(frontEnd, newFrontEndStake, msg.sender);
        ...
        // Update deposit
        uint newDeposit = compoundedDebtDeposit.sub(debtToWithdraw);
        _updateDepositAndSnapshots(msg.sender, newDeposit);
        emit UserDepositChanged(msg.sender, newDeposit);
    ...
    * - Updates snapshots for deposit and tagged front end stake */
    function withdrawETHGainToTrove(address _upperHint, address _lowerHint) external override {
        ...
        _updateDepositAndSnapshots(msg.sender, compoundedDebtDeposit);

        /* Emit events before transferring ETH gain to Trove.
         This lets the event log make more sense (i.e. so it appears that first the ETH gain is withdrawn
        and then it is deposited into the Trove, not the other way around). */
        emit CollateralGainWithdrawn(msg.sender, depositorETHGain, debtLoss);
        emit UserDepositChanged(msg.sender, compoundedDebtDeposit);
    ...
    function _updateG(uint _LQTYIssuance) internal {
        ...
        *
        */
        if (totalDebt == 0 || _LQTYIssuance == 0) {return;}

        uint LQTYPerUnitStaked;
        LQTYPerUnitStaked =_computeLQTYPerUnitStaked(_LQTYIssuance, totalDebt);
    ...
    function _computeLQTYPerUnitStaked(uint _LQTYIssuance, uint _totalDebtDeposits) internal returns (uint) {
        /*  
        ...
        */
        ...
        uint LQTYPerUnitStaked = LQTYNumerator.div(_totalDebtDeposits);
        lastLQTYError = LQTYNumerator.sub(LQTYPerUnitStaked.mul(_totalDebtDeposits));
    ...
    */
    function offset(uint _debtToOffset, uint _collToAdd) external override {
        ...
        uint totalDebt = totalDebtDeposits; // cached to save an SLOAD
        if (totalDebt == 0 || _debtToOffset == 0) { return; }

        _triggerLQTYIssuance(communityIssuance);

        (uint CollateralGainPerUnitStaked,
            uint DebtLossPerUnitStaked) = _computeRewardsPerUnitStaked(_collToAdd, _debtToOffset, totalDebt);

        _updateRewardSumAndProduct(CollateralGainPerUnitStaked, DebtLossPerUnitStaked);  // updates S and P
    ...
    function _computeRewardsPerUnitStaked(
        uint _collToAdd,
        uint _debtToOffset,
        uint _totalDebtDeposits
    )
        internal
        returns (uint collateralGainPerUnitStaked, uint debtLossPerUnitStaked)
    {
        /*
        * Compute the Debt and ETH rewards. Uses a "feedback" error correction, to keep
        ...
        * 5) Note: static analysis tools complain about this "division before multiplication", however, it is intended.
        */
        uint collateralNumerator = _collToAdd.mul(DECIMAL_PRECISION).add(lastETHError_Offset);
        ...
        if (_debtToOffset == _totalDebtDeposits) {
            ...
            uint debtLossNumerator = _debtToOffset.mul(DECIMAL_PRECISION).sub(lastDebtLossError_Offset);
            /*
            ...
            */
            debtLossPerUnitStaked = (debtLossNumerator.div(_totalDebtDeposits)).add(1);
            lastDebtLossError_Offset = (debtLossPerUnitStaked.mul(_totalDebtDeposits)).sub(debtLossNumerator);
        }

        collateralGainPerUnitStaked = collateralNumerator.div(_totalDebtDeposits);
        lastETHError_Offset = collateralNumerator.sub(collateralGainPerUnitStaked.mul(_totalDebtDeposits));

        return (collateralGainPerUnitStaked, debtLossPerUnitStaked);
    }

    // Update the Stability Pool reward sum S and product P
    function _updateRewardSumAndProduct(uint _CollateralGainPerUnitStaked, uint _DebtLossPerUnitStaked) internal {
        uint currentP = P;
        uint newP;

        assert(_DebtLossPerUnitStaked <= DECIMAL_PRECISION);
        /*
        ...
        */
        uint newProductFactor = uint(DECIMAL_PRECISION).sub(_DebtLossPerUnitStaked);
        ...
        */
        uint marginalETHGain = _CollateralGainPerUnitStaked.mul(currentP);
    ...
    function _decreaseDebt(uint _amount) internal {
        uint newtotalDebtDeposits = totalDebtDeposits.sub(_amount);
        totalDebtDeposits = newtotalDebtDeposits;
        emit StabilityPoolDebtBalanceUpdated(newtotalDebtDeposits);
    }
    ...
    */
    ...
    // --- Sender functions for Debt deposit, ETH gains and LQTY gains ---

    // Transfer the Debt tokens from the user to the Stability Pool's address, and update its recorded Debt
    function _sendDebttoStabilityPool(address _address, uint _amount) internal {
        debtToken.sendToPool(_address, address(this), _amount);
        uint newtotalDebtDeposits = totalDebtDeposits.add(_amount);
        totalDebtDeposits = newtotalDebtDeposits;
        emit StabilityPoolDebtBalanceUpdated(newtotalDebtDeposits);
    }
    ...
    // Send Debt to user and decrease Debt in Pool
    function _sendDebtToDepositor(address _depositor, uint DebtWithdrawal) internal {
        if (DebtWithdrawal == 0) {return;}

        debtToken.returnFromPool(address(this), _depositor, DebtWithdrawal);
        _decreaseDebt(DebtWithdrawal);
    }
    ...
    function _requireUserHasETHGain(address _depositor) internal view {
        uint ETHGain = getDepositorCollateralGain(_depositor);
