...
contract('BorrowerOperations', async accounts => {
  ...
  const testCorpus = ({ withProxy = false }) => {
    ...
    const users = [alice, bob, carol, dennis, whale, A, B, C, D, E, F, G, H, contracts.borrowerOperations.address];
    ...
    it("addColl(), active Trove: applies pending rewards and updates user's L_collateral, L_debt snapshots", async () => {
      // --- SETUP ---
      ...
      const alicePendingDebtReward = await troveManager.getPendingDebtReward(alice)
      const bobPendingDebtReward = await troveManager.getPendingDebtReward(bob)
      for (reward of [alicePendingCollateralReward, bobPendingCollateralReward, alicePendingDebtReward, bobPendingDebtReward]) {
      ...
      /* Check that both Alice and Bob's snapshots of the rewards-per-unit-staked metrics should be updated
       to the latest values of L_collateral and L_debt */
      ...
      assert.isAtMost(th.getDifference(alice_ETHrewardSnapshot_After, L_collateral), 100)
      assert.isAtMost(th.getDifference(alice_LUSDDebtRewardSnapshot_After, L_debt), 100)
      assert.isAtMost(th.getDifference(bob_ETHrewardSnapshot_After, L_collateral), 100)
      assert.isAtMost(th.getDifference(bob_LUSDDebtRewardSnapshot_After, L_debt), 100)
    })
    ...
    //   await borrowerOperations.openTrove(dec(15, 'ether'), th._100pct, dec(100, 18), alice, alice, { from: alice })
    //   await borrowerOperations.openTrove(dec(4, 'ether'), th._100pct, dec(100, 18), bob, bob, { from: bob })
    //   await borrowerOperations.openTrove(dec(5, 'ether'), th._100pct, dec(900, 18), carol, carol, { from: carol })

    //   await borrowerOperations.openTrove(dec(1, 'ether'), th._100pct, 0, dennis, dennis, { from: dennis })
    //   // --- TEST ---
    ...
    //   the ETH from her Trove has now become the totalPendingCollateralReward. So:

    //   totalStakes = (alice_Stake + bob_Stake + dennis_orig_stake ) = (15 + 4 + 1) =  20 ETH.
    //   totalCollateral = (alice_Collateral + bob_Collateral + dennis_orig_coll + totalPendingCollateralReward) = (15 + 4 + 1 + 5)  = 25 ETH.
    ...
    it("withdrawColl(): applies pending rewards and updates user's L_collateral, L_debt snapshots", async () => {
      // --- SETUP ---
      ...
      const pendingDebtReward_A = await troveManager.getPendingDebtReward(alice)
      const pendingCollReward_B = await troveManager.getPendingCollateralReward(bob)
      const pendingDebtReward_B = await troveManager.getPendingDebtReward(bob)
    ...
    it("withdrawLUSD(): decays a non-zero base rate", async () => {
      ...
      const A_LUSDBal = await debtToken.balanceOf(A)
    ...
    it("withdrawLUSD(): borrowing at non-zero base rate sends LUSD fee to LQTY staking contract", async () => {
      ...
      const lqtyStaking_LUSDBalance_Before = await debtToken.balanceOf(lqtyStaking.address)
      ...
      const lqtyStaking_LUSDBalance_After = await debtToken.balanceOf(lqtyStaking.address)
    ...
    it("withdrawLUSD(): Borrowing at non-zero base rate increases the LQTY staking contract LUSD fees-per-unit-staked", async () => {
      ...
      const F_Debt_Before = await lqtyStaking.F_Debt()
      ...
      assert.isTrue(F_Debt_After.gt(F_Debt_Before))
    })
    ...
    it("withdrawLUSD(): Borrowing at non-zero base rate sends requested amount to the user", async () => {
      ...
      const D_LUSDBalanceBefore = await debtToken.balanceOf(D)
      ...
      const D_LUSDBalanceAfter = await debtToken.balanceOf(D)
    ...
    it("withdrawLUSD(): Borrowing at zero base rate changes LUSD fees-per-unit-staked", async () => {
      ...
      const F_Debt_After = await lqtyStaking.F_Debt()
      assert.isTrue(F_Debt_After.gt('0'))
    })
    ...
    it("withdrawLUSD(): increases user DebtToken balance by correct amount", async () => {
    ...
    it("repayLUSD(): Succeeds when it would leave trove with net debt >= minimum net debt", async () => {
      ...
      await borrowerOperations.openTrove(dec(100, 30), th._100pct, await getNetBorrowingAmount(MIN_NET_DEBT.add(toBN('2'))), A, A, { from: A })
      ...
      await borrowerOperations.openTrove(dec(100, 30), th._100pct, dec(20, 25), B, B, { from: B })
    ...
    it("repayLUSD(): decreases user DebtToken balance by correct amount", async () => {
      await openTrove({ extraLUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraLUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      const aliceDebtBefore = await getTroveEntireDebt(alice)
      assert.isTrue(aliceDebtBefore.gt(toBN('0')))
      ...
      // check after
      const alice_DebtTokenBalance_After = await debtToken.balanceOf(alice)
      th.assertIsApproximatelyEqual(alice_DebtTokenBalance_After, alice_DebtTokenBalance_Before.sub(aliceDebtBefore.div(toBN(10))))
    })
    ...
    it("adjustTrove(): reverts when adjustment would leave trove with ICR < MCR", async () => {
     ...
     await assertRevert(borrowerOperations.adjustTrove(collTopUp, th._100pct, 0, LUSDRepayment, false, alice, alice, { from: alice }), 
      "BorrowerOps: An operation that would result in ICR < MCR is not permitted")
    })
    ...
    it("adjustTrove(): reverts if max fee < 0.5% in Normal mode", async () => {
      ...
      await assertRevert(borrowerOperations.adjustTrove(dec(2, 16), 0, 0, dec(1, 18), true, A, A, { from: A }), "Max fee percentage must be between 0.5% and 100%")
      await assertRevert(borrowerOperations.adjustTrove(dec(2, 16), 1, 0, dec(1, 18), true, A, A, { from: A }), "Max fee percentage must be between 0.5% and 100%")
      await assertRevert(borrowerOperations.adjustTrove(dec(2, 16), '4999999999999999', 0, dec(1, 18), true, A, A, { from: A }), "Max fee percentage must be between 0.5% and 100%")
    })
    ...
    it("adjustTrove(): allows max fee < 0.5% in Recovery mode", async () => {
      ...
      await collateralToken.mint(A, dec(3030300, 18));
      await collateralToken.approveInternal(A, borrowerOperations.address, dec(3030300, 18));
      await borrowerOperations.adjustTrove(dec(300, 18), 0, 0, dec(1, 9), true, A, A, { from: A })
      await priceFeed.setPrice(dec(1, 18))
      assert.isTrue(await th.checkRecoveryMode(contracts))
      await borrowerOperations.adjustTrove(dec(30000, 18), 1, 0, dec(1, 9), true, A, A, { from: A })
      await priceFeed.setPrice(dec(1, 16))
      assert.isTrue(await th.checkRecoveryMode(contracts))
      await borrowerOperations.adjustTrove(dec(3000000, 18), '4999999999999999', 0, dec(1, 9), true, A, A, { from: A })
    })
    ...
    it("adjustTrove(): decays a non-zero base rate", async () => {
      ...
      // D adjusts trove
      await borrowerOperations.adjustTrove(0, th._100pct, 0, dec(37, 18), true, D, D, { from: D })
      ...
      // E adjusts trove
      await borrowerOperations.adjustTrove(0, th._100pct, 0, dec(37, 15), true, E, E, { from: D })
    ...
    it("adjustTrove(): doesn't decay a non-zero base rate when user issues 0 debt", async () => {
      ...
      await collateralToken.approveInternal(D, borrowerOperations.address, dec(1, 18));
      await borrowerOperations.adjustTrove(dec(1, 18), th._100pct, 0, 0, false, D, D, { from: D })
    ...
    it("adjustTrove(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation", async () => {
      ...
      // Borrower C triggers a fee
      await borrowerOperations.adjustTrove(0, th._100pct, 0, dec(1, 18), true, C, C, { from: C })
    ...
    if (!withProxy) { // TODO: use rawLogs instead of logs
      it("adjustTrove(): borrowing at non-zero base records the (drawn debt + fee) on the Trove struct", async () => {
        ...
        // D withdraws LUSD
        const adjustmentTx = await borrowerOperations.adjustTrove(0, th._100pct, 0, withdrawal_D, true, D, D, { from: D })
    ...
    it("adjustTrove(): Borrowing at non-zero base rate sends requested amount to the user", async () => {
      ...
      await borrowerOperations.adjustTrove(0, th._100pct, 0, LUSDRequest_D, true, D, D, { from: D })
    ...
    it("adjustTrove(): Borrowing at zero base rate changes LQTY staking contract LUSD fees-per-unit-staked", async () => {
      ...
      // Check staking LUSD balance before == 0
      const F_Debt_Before = await lqtyStaking.F_Debt()
      assert.isTrue(F_Debt_Before.eq(toBN('0')))
    ...
    it("adjustTrove(): Borrowing at zero base rate sends total requested LUSD to the user", async () => {
      ...
      const D_LUSDBalBefore = await debtToken.balanceOf(D)
      // Check baseRate is zero
      const baseRate_1 = await troveManager.baseRate()
      assert.equal(baseRate_1, '0')
      ...
      const DUSDBalanceBefore = await debtToken.balanceOf(D)
      ...
      const LUSDBalanceAfter = await debtToken.balanceOf(D)
    ...
    it("adjustTrove(): reverts when calling address has no active trove", async () => {
      ...
      await collateralToken.mint(alice, dec(1, 18));
      await collateralToken.approveInternal(alice, borrowerOperations.address, dec(1, 18));
      await borrowerOperations.adjustTrove( dec(1, 18), th._100pct, 0, dec(50, 18), true, alice, alice, { from: alice})

      await collateralToken.mint(carol, dec(1, 18));
      await collateralToken.approveInternal(carol, borrowerOperations.address, dec(1, 18));
      try {
        const txCarol = await borrowerOperations.adjustTrove( dec(1, 18), th._100pct, 0, dec(50, 18), true, carol, carol, { from: carol })
        assert.isFalse(txCarol.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("adjustTrove(): reverts in Recovery Mode when the adjustment would reduce the TCR", async () => {
      ...
      await collateralToken.mint(alice, dec(1, 18));
      await collateralToken.approveInternal(alice, borrowerOperations.address, dec(1, 18));
      const txAlice = await borrowerOperations.adjustTrove(dec(1, 18), th._100pct, 0, dec(50, 18), true, alice, alice, { from: alice })
      assert.isTrue(txAlice.receipt.status)
      ...
      try { // collateral withdrawal should also fail
        const txAlice = await borrowerOperations.adjustTrove(0, th._100pct, dec(1, 'ether'), 0, false, alice, alice, { from: alice })
        assert.isFalse(txAlice.receipt.status)
      } catch (err) {
        assert.include(err.message, "Recovery Mode")
      }

      try { // debt increase should fail
        const txBob = await borrowerOperations.adjustTrove(0, th._100pct, 0, dec(50, 18), true, bob, bob, { from: bob })
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
        assert.include(err.message, "ICR >= CCR")
      }

      try { // debt increase that's also a collateral increase should also fail, if ICR will be worse off
        const txBob = await borrowerOperations.adjustTrove(dec(1, 'ether'), th._100pct, 0, dec(111, 18), true, bob, bob, { from: bob })
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
        assert.include(err.message, "ICR >= CCR")
      }
    ...
    it("adjustTrove(): collateral withdrawal reverts in Recovery Mode", async () => {
      ...
      await assertRevert(borrowerOperations.adjustTrove(0, th._100pct, 1, dec(5000, 18), false, alice, alice, { from: alice }),
    ...
    it("adjustTrove(): debt increase that would leave ICR < 150% reverts in Recovery Mode", async () => {
      ...
      await assertRevert(borrowerOperations.adjustTrove(collIncrease, th._100pct, 0, debtIncrease, true, alice, alice, { from: alice }),
    ...
    it("adjustTrove(): debt increase that would reduce the ICR reverts in Recovery Mode", async () => {
      ...
      await assertRevert(borrowerOperations.adjustTrove(aliceCollIncrease, th._100pct, 0, aliceDebtIncrease, true, alice, alice, { from: alice }),
      ...
      await assertRevert(borrowerOperations.adjustTrove(bobCollIncrease, th._100pct, 0, bobDebtIncrease, true, bob, bob, { from: bob }),
        " BorrowerOps: Operation must leave trove with ICR >= CCR")
    })
    ...
    it("adjustTrove(): A trove with ICR < CCR in Recovery Mode can adjust their trove to ICR > CCR", async () => {
      ...
      await collateralToken.mint(alice, collIncrease);
      await collateralToken.approveInternal(alice, borrowerOperations.address, collIncrease);
      const tx = await borrowerOperations.adjustTrove(collIncrease, th._100pct, 0, debtIncrease, true, alice, alice, { from: alice })
      assert.isTrue(tx.receipt.status)
    ...
    it("adjustTrove(): debt increase in Recovery Mode charges no fee", async () => {
      ...
      const lqtyStakingLUSDBalanceBefore = await debtToken.balanceOf(lqtyStaking.address)
      assert.isTrue(lqtyStakingLUSDBalanceBefore.gt(toBN('0')))

      await collateralToken.mint(alice, dec(100, 18));
      await collateralToken.approveInternal(alice, borrowerOperations.address, dec(100, 18));
      const txAlice = await borrowerOperations.adjustTrove(dec(100, 18), th._100pct, 0, dec(50, 18), true, alice, alice, { from: alice })
      assert.isTrue(txAlice.receipt.status)
      ...
      const lqtyStakingLUSDBalanceAfter = await debtToken.balanceOf(lqtyStaking.address)
    ...
    it("adjustTrove(): reverts when change would cause the TCR of the system to fall below the CCR", async () => {
      ...
      try {
        const txBob = await borrowerOperations.adjustTrove(0, th._100pct, 0, dec(1, 18), true, bob, bob, { from: bob })
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("adjustTrove(): reverts when LUSD repaid is > debt of the trove", async () => {
      ...
      await debtToken.transfer(bob, bobFee, { from: alice })

      const remainingDebt = (await troveManager.getTroveDebt(bob)).sub(DEBT_GAS_COMPENSATION)

      // Bob attempts an adjustment that would repay 1 wei more than his debt
      await assertRevert(
        borrowerOperations.adjustTrove(dec(1, 'ether'), th._100pct, 0, remainingDebt.add(toBN(1)), false, bob, bob, { from: bob }),
        "revert"
      )
    })
    ...
    it("adjustTrove(): reverts when attempted ETH withdrawal is >= the trove's collateral", async () => {
      ...
      try {
        const txCarol = await borrowerOperations.adjustTrove(0, th._100pct, carolColl.add(toBN(1)), 0, true, carol, carol, { from: carol })
        assert.isFalse(txCarol.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("adjustTrove(): reverts when change would cause the ICR of the trove to fall below the MCR", async () => {
      ...
      try {
        const txBob = await borrowerOperations.adjustTrove(dec(1, 'ether'), th._100pct, 0, dec(100, 18), true, bob, bob, { from: bob })
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("adjustTrove(): With 0 coll change, doesnt change borrower's coll or ActivePool coll", async () => {
      ...
      await borrowerOperations.adjustTrove(0, th._100pct, 0, dec(50, 18), true, alice, alice, { from: alice})
    ...
    it("adjustTrove(): With 0 debt change, doesnt change borrower's debt or ActivePool debt", async () => {
      ...
      await collateralToken.mint(alice, dec(1, 18));
      await collateralToken.approveInternal(alice, borrowerOperations.address, dec(1, 18));
      await borrowerOperations.adjustTrove(dec(1, 18), th._100pct, 0, 0, false, alice, alice, { from: alice })
    ...
    it("adjustTrove(): updates borrower's debt and coll with an increase in both", async () => {
      ...
      await collateralToken.mint(alice, dec(1, 18));
      await collateralToken.approveInternal(alice, borrowerOperations.address, dec(1, 18));
      await borrowerOperations.adjustTrove(dec(1, 18), th._100pct, 0, await getNetBorrowingAmount(dec(50, 18)), true, alice, alice, { from: alice })
    ...
    it("adjustTrove(): updates borrower's debt and coll with a decrease in both", async () => {
      ...
      await borrowerOperations.adjustTrove(0, th._100pct, dec(500, 'finney'), dec(50, 18), false, alice, alice, { from: alice })
    ...
    it("adjustTrove(): updates borrower's  debt and coll with coll increase, debt decrease", async () => {
      ...
      await collateralToken.mint(alice, dec(5, 17));
      await collateralToken.approveInternal(alice, borrowerOperations.address, dec(5, 17));
      await borrowerOperations.adjustTrove(dec(5, 17), th._100pct, 0, dec(50, 18), false, alice, alice, { from: alice })
    ...
    it("adjustTrove(): updates borrower's debt and coll with coll decrease, debt increase", async () => {
      ...
      await borrowerOperations.adjustTrove(0, th._100pct, dec(1, 17), await getNetBorrowingAmount(dec(1, 18)), true, alice, alice, { from: alice })
    ...
    it("adjustTrove(): changes DebtToken balance by the requested decrease", async () => {
      ...
      // Alice adjusts trove - coll decrease and debt decrease
      await borrowerOperations.adjustTrove(0, th._100pct, dec(100, 'finney'), dec(10, 18), false, alice, alice, { from: alice })

      // check after
      const alice_DebtTokenBalance_After = await debtToken.balanceOf(alice)
      assert.isTrue(alice_DebtTokenBalance_After.eq(alice_DebtTokenBalance_Before.sub(toBN(dec(10, 18)))))
    })
    ...
    it("adjustTrove(): changes DebtToken balance by the requested increase", async () => {
      ...
      await collateralToken.mint(alice, dec(1, 18));
      await collateralToken.approveInternal(alice, borrowerOperations.address, dec(1, 18));
      await borrowerOperations.adjustTrove(dec(1, 18), th._100pct, 0, dec(100, 18), true, alice, alice, { from: alice })

      // check after
      const alice_DebtTokenBalance_After = await debtToken.balanceOf(alice)
      assert.isTrue(alice_DebtTokenBalance_After.eq(alice_DebtTokenBalance_Before.add(toBN(dec(100, 18)))))
    })
    ...
    it("adjustTrove(): Changes the LUSD debt in ActivePool by requested decrease", async () => {
      ...
      await collateralToken.mint(alice, dec(1, 18));
      await collateralToken.approveInternal(alice, borrowerOperations.address, dec(1, 18));
      await borrowerOperations.adjustTrove(dec(1, 18), th._100pct, 0, dec(30, 18), false, alice, alice, { from: alice })
    ...
    it("adjustTrove(): Changes the LUSD debt in ActivePool by requested increase", async () => {
      ...
      await collateralToken.mint(alice, dec(1, 18));
      await collateralToken.approveInternal(alice, borrowerOperations.address, dec(1, 18));
      await borrowerOperations.adjustTrove(dec(1, 18), th._100pct, 0, await getNetBorrowingAmount(dec(100, 18)), true, alice, alice, { from: alice })
    ...
    it("adjustTrove(): new coll = 0 and new debt = 0 is not allowed, as gas compensation still counts toward ICR", async () => {
      ...
      await assertRevert(
        borrowerOperations.adjustTrove(0, th._100pct, aliceColl, aliceDebt, true, alice, alice, { from: alice }),
        'BorrowerOps: An operation that would result in ICR < MCR is not permitted'
      )
    })
    ...
    it("adjustTrove(): Reverts if requested debt increase and amount is zero", async () => {
      ...
      await assertRevert(borrowerOperations.adjustTrove(0, th._100pct, 0, 0, true, alice, alice, { from: alice }),
    ...
    it("adjustTrove(): Reverts if requested coll withdrawal and ether is sent", async () => {
      ...
      await assertRevert(borrowerOperations.adjustTrove(dec(3, 'ether'), th._100pct, dec(1, 'ether'), dec(100, 18), true, alice, alice, { from: alice }), 'BorrowerOperations: Cannot withdraw and add coll')
    })

    it("adjustTrove(): Reverts if it’s zero adjustment", async () => {
      await openTrove({ extraLUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: alice } })

      await assertRevert(borrowerOperations.adjustTrove(0, th._100pct, 0, 0, false, alice, alice, { from: alice }),
    ...
    it("adjustTrove(): Reverts if requested coll withdrawal is greater than trove's collateral", async () => {
      ...
      await assertRevert(borrowerOperations.adjustTrove(0, th._100pct, aliceColl.add(toBN(1)), 0, false, alice, alice, { from: alice }))
      await assertRevert(borrowerOperations.adjustTrove(0, th._100pct, aliceColl.add(toBN(dec(37, 'ether'))), 0, false, bob, bob, { from: bob }))
    })
    ...
    it("adjustTrove(): Reverts if borrower has insufficient LUSD balance to cover his debt repayment", async () => {
      ...
      const B_LUSDBal = await debtToken.balanceOf(B)
      assert.isTrue(B_LUSDBal.lt(bobDebt))

      const repayLUSDPromise_B = borrowerOperations.adjustTrove(0, th._100pct, 0, bobDebt, false, B, B, { from: B })
    ...
    it("closeTrove(): reduces a Trove's collateral to zero", async () => {
      ...
      const aliceCollBefore = await getTroveEntireColl(alice)
      const dennisLUSD = await debtToken.balanceOf(dennis)
      assert.isTrue(aliceCollBefore.gt(toBN('0')))
      assert.isTrue(dennisLUSD.gt(toBN('0')))

      // To compensate borrowing fees
      await debtToken.transfer(alice, dennisLUSD.div(toBN(2)), { from: dennis })
    ...
    it("closeTrove(): zero's the troves reward snapshots", async () => {
      ...
      const L_debt_A_Snapshot = (await troveManager.rewardSnapshots(alice))[1]
      ...
      const L_debt_Snapshot_A_AfterLiquidation = (await troveManager.rewardSnapshots(alice))[1]
      ...
      const L_debt_Snapshot_A_afterAliceCloses = (await troveManager.rewardSnapshots(alice))[1]
    ...
    it("closeTrove(): subtracts the debt of the closed Trove from the Borrower's DebtToken balance", async () => {
      ...
      const alice_LUSDBalance_Before = await debtToken.balanceOf(alice)
      ...
      const alice_LUSDBalance_After = await debtToken.balanceOf(alice)
      th.assertIsApproximatelyEqual(alice_LUSDBalance_After, alice_LUSDBalance_Before.sub(aliceDebt.sub(DEBT_GAS_COMPENSATION)))
    })
    ...
    it("closeTrove(): applies pending rewards", async () => {
      ...
      await borrowerOperations.adjustTrove(0, th._100pct, 0, dec(1, 18), true, whale, whale, { from: whale })
    ...
    it("openTrove(): Opens a trove with net debt >= minimum net debt", async () => {
      ...
      const txA = await borrowerOperations.openTrove(dec(100, 30), th._100pct, await getNetBorrowingAmount(MIN_NET_DEBT.add(toBN(1))), A, A, { from: A})
      assert.isTrue(txA.receipt.status)
      assert.isTrue(await sortedTroves.contains(A))

      const txC = await borrowerOperations.openTrove(dec(100, 30), th._100pct, await getNetBorrowingAmount(MIN_NET_DEBT.add(toBN(dec(47789898, 22)))), A, A, { from: C })
      assert.isTrue(txC.receipt.status)
      assert.isTrue(await sortedTroves.contains(C))
    })

    it("openTrove(): reverts if net debt < minimum net debt", async () => {
      const txAPromise = borrowerOperations.openTrove(dec(100, 30), th._100pct, 0, A, A, { from: A })
      await assertRevert(txAPromise, "revert")

      const txBPromise = borrowerOperations.openTrove(dec(100, 30), th._100pct, await getNetBorrowingAmount(MIN_NET_DEBT.sub(toBN(1))), B, B, { from: B })
      await assertRevert(txBPromise, "revert")

      const txCPromise = borrowerOperations.openTrove(dec(100, 30), th._100pct, MIN_NET_DEBT.sub(toBN(dec(173, 18))), C, C, { from: C })
    ...
    it("openTrove(): reverts if max fee > 100%", async () => {
      await assertRevert(borrowerOperations.openTrove(dec(1000, 'ether'), dec(2, 18), dec(10000, 18), A, A, { from: A }), "Max fee percentage must be between 0.5% and 100%")
      await assertRevert(borrowerOperations.openTrove(dec(1000, 'ether'), '1000000000000000001', dec(20000, 18), B, B, { from: B }), "Max fee percentage must be between 0.5% and 100%")
    })

    it("openTrove(): reverts if max fee < 0.5% in Normal mode", async () => {
      await assertRevert(borrowerOperations.openTrove(dec(1200, 'ether'), 0, dec(195000, 18), A, A, { from: A }), "Max fee percentage must be between 0.5% and 100%")
      await assertRevert(borrowerOperations.openTrove(dec(1000, 'ether'), 1, dec(195000, 18), A, A, { from: A,  }), "Max fee percentage must be between 0.5% and 100%")
      await assertRevert(borrowerOperations.openTrove(dec(1200, 'ether'), '4999999999999999', dec(195000, 18), B, B, { from: B}), "Max fee percentage must be between 0.5% and 100%")
    })

    it("openTrove(): allows max fee < 0.5% in Recovery Mode", async () => {
      await borrowerOperations.openTrove(dec(2000, 'ether'), th._100pct, dec(195000, 18), A, A, { from: A })

      await priceFeed.setPrice(dec(100, 18))
      assert.isTrue(await th.checkRecoveryMode(contracts))

      await borrowerOperations.openTrove(dec(3100, 'ether'), 0, dec(19500, 18), B, B, { from: B })
      await priceFeed.setPrice(dec(50, 18))
      assert.isTrue(await th.checkRecoveryMode(contracts))
      await borrowerOperations.openTrove(dec(3100, 'ether'), 1, dec(19500, 18), C, C, { from: C})
      await priceFeed.setPrice(dec(25, 18))
      assert.isTrue(await th.checkRecoveryMode(contracts))
      await borrowerOperations.openTrove( dec(3100, 'ether'), '4999999999999999', dec(19500, 18), D, D, { from: D })
    })
    ...
    it("openTrove(): reverts if fee exceeds max fee percentage", async () => {
      ...
      const lessThan5pct = '49999999999999999'
      await assertRevert(borrowerOperations.openTrove( dec(1000, 'ether'), lessThan5pct, dec(30000, 18), A, A, { from: D }), "Fee exceeded provided maximum")

      borrowingRate = await troveManager.getBorrowingRate() // expect 5% rate
      assert.equal(borrowingRate, dec(5, 16))
      // Attempt with maxFee 1%
      await assertRevert(borrowerOperations.openTrove(dec(1000, 'ether') , dec(1, 16), dec(30000, 18), A, A, { from: D}), "Fee exceeded provided maximum")

      borrowingRate = await troveManager.getBorrowingRate() // expect 5% rate
      assert.equal(borrowingRate, dec(5, 16))
      // Attempt with maxFee 3.754%
      await assertRevert(borrowerOperations.openTrove(dec(1000, 'ether'), dec(3754, 13), dec(30000, 18), A, A, { from: D }), "Fee exceeded provided maximum")

      borrowingRate = await troveManager.getBorrowingRate() // expect 5% rate
      assert.equal(borrowingRate, dec(5, 16))
      // Attempt with maxFee 1e-16%
      await assertRevert(borrowerOperations.openTrove(dec(1000, 'ether'), dec(5, 15), dec(30000, 18), A, A, { from: D }), "Fee exceeded provided maximum")
    })
    ...
    it("openTrove(): succeeds when fee is less than max fee percentage", async () => {
      ...
      // Attempt with maxFee > 5%
      const moreThan5pct = '50000000000000001'
      const tx1 = await borrowerOperations.openTrove(dec(100, 'ether'), moreThan5pct, dec(10000, 18), A, A, { from: D })
      assert.isTrue(tx1.receipt.status)
      ...
      // Attempt with maxFee = 5%
      const tx2 = await borrowerOperations.openTrove(dec(100, 'ether'), dec(5, 16), dec(10000, 18), A, A, { from: H})
      assert.isTrue(tx2.receipt.status)
      ...
      // Attempt with maxFee 10%
      const tx3 = await borrowerOperations.openTrove(dec(100, 'ether'), dec(1, 17), dec(10000, 18), A, A, { from: E})
      assert.isTrue(tx3.receipt.status)
      ...
      // Attempt with maxFee 37.659%
      const tx4 = await borrowerOperations.openTrove(dec(100, 'ether'), dec(37659, 13), dec(10000, 18), A, A, { from: F })
      assert.isTrue(tx4.receipt.status)

      // Attempt with maxFee 100%
      const tx5 = await borrowerOperations.openTrove(dec(100, 'ether'), dec(1, 18), dec(10000, 18), A, A, { from: G })
      assert.isTrue(tx5.receipt.status)
    })
    ...
    if (!withProxy) { // TODO: use rawLogs instead of logs
      it("openTrove(): borrowing at non-zero base records the (drawn debt + fee  + liq. reserve) on the Trove struct", async () => {
        ...
        // D withdraws LUSD
        const openTroveTx = await borrowerOperations.openTrove(dec(200, 'ether'), th._100pct, D_LUSDRequest, ZERO_ADDRESS, ZERO_ADDRESS, { from: D })
        ...
        th.assertIsApproximatelyEqual(newDebt, D_LUSDRequest.add(emittedFee).add(DEBT_GAS_COMPENSATION), 100000)
      })
    }
    ...
    it("openTrove(): Borrowing at non-zero base rate sends requested amount to the user", async () => {
      ...
      await borrowerOperations.openTrove(dec(500, 'ether'), th._100pct, LUSDRequest_D, D, D, { from: D })
      ...
      // Check D's LUSD balance now equals their requested LUSD
      const LUSDBalance_D = await debtToken.balanceOf(D)
    ...
    it("openTrove(): Borrowing at zero base rate changes the LQTY staking contract LUSD fees-per-unit-staked", async () => {
      ...
      // Check LUSD reward per LQTY staked > 0
      const F_Debt_After = await lqtyStaking.F_Debt()
      assert.isTrue(F_Debt_After.gt(toBN('0')))
    })
    ...
    it("openTrove(): Borrowing at zero base rate charges minimum fee", async () => {
      ...
      const txC = await borrowerOperations.openTrove(dec(100, 'ether'), th._100pct, LUSDRequest, ZERO_ADDRESS, ZERO_ADDRESS, { from: C })
    ...
    it("openTrove(): Reverts opening a trove with min debt when system is in Recovery Mode", async () => {
      ...
      await assertRevert(borrowerOperations.openTrove(dec(1, 'ether'), th._100pct, await getNetBorrowingAmount(MIN_NET_DEBT), carol, carol, { from: carol}))
    })
    ...
    it("openTrove(): creates a new Trove and assigns the correct collateral and debt amount", async () => {
      ...
      borrowerOperations.openTrove(dec(100, 'ether'), th._100pct, MIN_NET_DEBT, carol, carol, { from: alice })
    ...
    it("openTrove(): records up-to-date initial snapshots of L_collateral and L_debt", async () => {
      // --- SETUP ---
      ...
      /* with total stakes = 10 ether, after liquidation, L_collateral should equal 1/10 ether per-ether-staked,
       and L_LUSD should equal 18 LUSD per-ether-staked. */

      const L_collateral = await troveManager.L_collateral()
      const L_LUSD = await troveManager.L_debt()
      ...
      assert.isAtMost(th.getDifference(bob_ETHrewardSnapshot, L_collateral), 1000)
    ...
    it("openTrove(): increases the Trove's LUSD debt by the correct amount", async () => {
      ...
      await borrowerOperations.openTrove(dec(100, 'ether'), th._100pct, await getOpenTroveLUSDAmount(dec(10000, 18)), alice, alice, { from: alice})
    ...
    it("openTrove(): increases user DebtToken balance by correct amount", async () => {
      ...
      await borrowerOperations.openTrove(dec(100, 'ether'), th._100pct, dec(10000, 18), alice, alice, { from: alice})
    ...
    it("getCompositeDebt(): returns debt + gas comp", async () => {
      ...
      assert.equal(res1, DEBT_GAS_COMPENSATION.toString())

      const res2 = await borrowerOperations.getCompositeDebt(dec(90, 18))
      th.assertIsApproximatelyEqual(res2, DEBT_GAS_COMPENSATION.add(toBN(dec(90, 18))))

      const res3 = await borrowerOperations.getCompositeDebt(dec(24423422357345049, 12))
      th.assertIsApproximatelyEqual(res3, DEBT_GAS_COMPENSATION.add(toBN(dec(24423422357345049, 12))))
    })
    ...
    describe("getNewTCRFromTroveChange() returns the correct TCR", async () => {
      ...
      it("collChange = 0, debtChange = 0", async () => {
        ...
        await borrowerOperations.openTrove(troveColl, th._100pct, troveLUSDAmount, alice, alice, { from: alice})
        await borrowerOperations.openTrove(troveColl, th._100pct, troveLUSDAmount, bob, bob, { from: bob })
    ...
    if (!withProxy) {
      it.skip('closeTrove(): fails if owner cannot receive ETH', async () => {
        ...
        await borrowerOperations.openTrove(dec(1000, 18), th._100pct, dec(100000, 18), alice, alice, { from: alice})

        // Alice sends LUSD to NonPayable so its LUSD balance covers its debt
        await debtToken.transfer(nonPayable.address, dec(10000, 18), {from: alice})

        // open trove from NonPayable proxy contract
        const _100pctHex = '0xde0b6b3a7640000'
        const _1e25Hex = '0xd3c21bcecceda1000000'
        const openTroveData = th.getTransactionData('openTrove(uint256,uint256,address,address,uint256)', [_100pctHex, _1e25Hex, '0x0', '0x0',_100pctHex])
